...
class TriggersPage(BasePage):
    ...
    async def verify_on_page(self) -> bool:
        ...
        Waits for the URL to match triggers/all-namespaces or triggers/ns/<namespace>, then checks if
        the Triggers header is visible.
        ...
        """
        return await self._verify_page_regex(TRIGGERS_URL, self.locators.TRIGGERS_HEADER, "Triggers page")

    async def navigate_to_event_listeners_tab(self) -> bool:
        """
        ...
        """
        return await self.click_element(self.locators.EVENT_LISTENERS_TAB)

    async def navigate_to_trigger_templates_tab(self) -> bool:
        """
        Navigates to the TriggerTemplates tab on the Triggers page by clicking on the TriggerTemplates tab.
        Uses click_element() which waits up to the configured timeout for the element to be clickable.
        :return: bool: True if tab click succeeds, False if click fails or raises TimeoutError.
        """
        return await self.click_element(self.locators.TRIGGER_TEMPLATES_TAB)

    async def navigate_to_trigger_bindings_tab(self) -> bool:
        """
        Navigates to the TriggerBindings tab on the Triggers page by clicking on the TriggerBindings tab.
        Uses click_element() which waits up to the configured timeout for the element to be clickable.
        :return: bool: True if tab click succeeds, False if click fails or raises TimeoutError.
        """
        return await self.click_element(self.locators.TRIGGER_BINDINGS_TAB)

    async def navigate_to_cluster_trigger_bindings_tab(self) -> bool:
        """
        Navigates to the ClusterTriggerBindings tab on the Triggers page by clicking the tab.
        Uses click_element() which waits up to the configured timeout for the element to be clickable.
        :return: bool: True if tab click succeeds, False if click fails or raises TimeoutError.
        """
        return await self.click_element(self.locators.CLUSTER_TRIGGER_BINDINGS_TAB)
