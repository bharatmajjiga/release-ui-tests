...
from framework.locators.triggers import EventListenerYamlPageLocators
...
from framework.ui_components.console_url_patterns import EVENTLISTENER_YAML_URL


class EventListenerYamlPage(BasePage):
    """Page object for the EventListener YAML editor tab."""

    def __init__(self, page: Page, config: Config) -> None:
        super().__init__(page, config)
        self.locators = EventListenerYamlPageLocators()
    ...
    async def verify_on_page(self) -> bool:
        """
        Verifies that the EventListener YAML tab is currently displayed by checking URL and
        ...
        return await self._verify_page_regex(
            EVENTLISTENER_YAML_URL, self.locators.YAML_EDITOR, "EventListener YAML page"
        )
    ...
    async def click_save(self) -> bool:
        """
        Clicks the 'Save' button to save changes to the EventListener YAML.
        :return: bool: True if click succeeds.
        """
