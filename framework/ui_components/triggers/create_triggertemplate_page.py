...
from framework.locators.triggers import CreateTriggerTemplatePageLocators
...
class CreateTriggerTemplatePage(BasePage):
    """Page object for the Create TriggerTemplate YAML editor page."""

    def __init__(self, page: Page, config: Config) -> None:
        super().__init__(page, config)
        self.locators = CreateTriggerTemplatePageLocators()
        self.project_selector = ProjectSelector(page, config)
        self.favorites = Favorites(page, config)

    async def verify_on_page(self) -> bool:
        """
        Verifies that the Create TriggerTemplate page is currently displayed by checking URL and header visibility.
        Waits for the URL to contain 'triggers.tekton.dev~v1beta1~TriggerTemplate/~new', then checks if the
        Create TriggerTemplate header is visible.
        :return: bool: True if URL matches and Create TriggerTemplate header is visible.
        ...
        return await self.wait_for_url_to_contain(
            "triggers.tekton.dev~v1beta1~TriggerTemplate/~new"
        ) and await self.is_visible(self.locators.CREATE_TRIGGERTEMPLATE_HEADER)
    ...
    async def click_create(self) -> bool:
        """
        Clicks the 'Create' button to submit the YAML and create the TriggerTemplate resource.
        :return: bool: True if click succeeds.
        """
