...
from framework.locators.triggers import TriggerBindingYamlPageLocators
...
from framework.ui_components.console_url_patterns import TRIGGERBINDING_YAML_URL


class TriggerBindingYamlPage(BasePage):
    """Page object for the TriggerBinding YAML editor tab."""

    def __init__(self, page: Page, config: Config) -> None:
        super().__init__(page, config)
        self.locators = TriggerBindingYamlPageLocators()
    ...
    async def verify_on_page(self) -> bool:
        """
        Verifies that the TriggerBinding YAML tab is currently displayed by checking URL and
        ...
        return await self._verify_page_regex(
            TRIGGERBINDING_YAML_URL, self.locators.YAML_EDITOR, "TriggerBinding YAML page"
        )
    ...
    async def click_save(self) -> bool:
        """
        Clicks the 'Save' button to save changes to the TriggerBinding YAML.
        :return: bool: True if click succeeds.
        """
