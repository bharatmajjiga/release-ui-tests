...
from framework.locators.triggers import CreateEventListenerPageLocators
...
class CreateEventListenerPage(BasePage):
    """Page object for the Create EventListener YAML editor page."""

    def __init__(self, page: Page, config: Config) -> None:
        super().__init__(page, config)
        self.locators = CreateEventListenerPageLocators()
        self.project_selector = ProjectSelector(page, config)
        self.favorites = Favorites(page, config)

    async def verify_on_page(self) -> bool:
        """
        Verifies that the Create EventListener page is currently displayed by checking URL and header visibility.
        Waits for the URL to contain 'triggers.tekton.dev~v1beta1~EventListener/~new', then checks if the
        Create EventListener header is visible.
        :return: bool: True if URL matches and Create EventListener header is visible.
        ...
        return await self.wait_for_url_to_contain(
            "triggers.tekton.dev~v1beta1~EventListener/~new"
        ) and await self.is_visible(self.locators.CREATE_EVENTLISTENER_HEADER)
    ...
    async def click_create(self) -> bool:
        """
        Clicks the 'Create' button to submit the YAML and create the EventListener resource.
        :return: bool: True if click succeeds.
        """
